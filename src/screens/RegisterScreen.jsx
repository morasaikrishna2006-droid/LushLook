...
const RegisterScreen = () => {
  ...
  const handleRegister = async (e) => {
    ...
    if (error) {
      ...
      // On successful registration, you might want to show a "check your email" message
      // or navigate to a profile completion page. For now, we'll navigate to the dashboard.
  ...
  const handleOAuthSignIn = async (provider) => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      ...
      options: {
        data: {
          // Pass the selected user type during social sign-up
          user_type: userType,
        },
        // The redirectTo URL is required for OAuth flows to work correctly
        redirectTo: window.location.origin,
        // Note: full_name is automatically populated by most OAuth providers
  ...
  return (
    <div className="bg-background min-h-screen flex items-center justify-center py-12">
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        <h2 className="text-2xl font-heading text-accent text-center mb-6">Create Your Account</h2>

        {error && <p className="bg-warning text-white text-center p-3 rounded-lg mb-6">{error}</p>}

        <form onSubmit={handleRegister} className="space-y-6">
          <input type="text" placeholder="Full Name" value={fullName} onChange={(e) => setFullName(e.target.value)} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary" />
          ...
          <div>
            ...
            <select id="userType" value={userType} onChange={(e) => setUserType(e.target.value)} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary">
              <option value="customer">Customer (Looking for services)</option>
              <option value="beautician">Beautician (Offering services)</option>
            </select>
          </div>
        ...
        <div className="flex items-center justify-center space-x-4">
          <button type="button" aria-label="Sign up with Google" onClick={() => handleOAuthSignIn('google')} disabled={loading} className="p-3 border rounded-full hover:bg-gray-100 disabled:opacity-50">
          ...
          <button type="button" aria-label="Sign up with Facebook" onClick={() => handleOAuthSignIn('facebook')} disabled={loading} className="p-3 border rounded-full hover:bg-gray-100 disabled:opacity-50">
